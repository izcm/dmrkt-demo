...
To resolve the clipboard issue, get the frontend to run in safe context by accessing it through SSH port forwarding:
...
| `.env.runtime`  | Auto-generated by `make demo-prepare` — don'tdo u see an edit |
...
- **prepare** — computes fork start-block and simulation window, derives the marketplace contract address and writes these to config / .env files
